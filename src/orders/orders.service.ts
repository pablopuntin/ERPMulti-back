//ESTE ORDER YA ABRE CAJA Y HACE EL MOVIMIENTO EN CAJA PARA REGISTRAR EL PAGO Y DESCONTAR STOCK
//refactor con apertura automatica de caja
import {
  Injectable,
  NotFoundException,
  BadRequestException
...
export class OrdersService {
  ...
  private async getEligibleCreditCustomer(order: Order) {
    if (!order.customerId) {
      return null;
    }
    ...
    if (!customer || !customer.isActive) {
      throw new BadRequestException(
        'La venta requiere un cliente válido para operar con cuenta corriente'
      );
    }
  ...
  private async registerPaymentIfNeeded(
    ...
    return this.orderRepo.manager.transaction(async (manager) => {
      ...
      const eligibleCreditCustomer =
        await this.getEligibleCreditCustomer(lockedOrder);
      ...
      if (isCurrentAccount) {
        if (!eligibleCreditCustomer) {
          throw new BadRequestException(
            'La cuenta corriente solo está disponible para clientes registrados y habilitados'
          );
        }
        ...
        if (requestedPaymentAmount > 0) {
          ...
          if (!paidByUserId) {
            throw new BadRequestException(
              'No hay usuario válido para registrar el pago'
            );
          }
        ...
        return requestedPaymentAmount > 0;
      }

      if (!requestedPaymentAmount || requestedPaymentAmount <= 0) {
        if (eligibleCreditCustomer) {
          return false;
        }

        throw new BadRequestException(
          'El remito requiere pago total para finalizar'
        );
      }
  ...
  async finalizeInCash(
    ...
    const hasDeliveryChanges =
      beforeDeliveryDelivered !== targetDeliveryDelivered;
    ...
    const reviewedOrder = await this.reviewInCash(userScope, id, reviewPayload);
    ...
    if (hasPendingItems && !isCurrentAccount) {
      ...
      const eligibleCreditCustomer =
        await this.getEligibleCreditCustomer(reviewedOrder);

      if (
        this.roundCurrency(
          requestedPaymentAmount - pendingAmountAfterReview
        ) !== 0 &&
        !eligibleCreditCustomer
      ) {
        throw new BadRequestException(
    ...
    const paymentRegistered = await this.registerPaymentIfNeeded(
      userScope,
      reviewedOrder,
      dto
    );
    ...
    if (hasDeliveryChanges) {
      finalOrder = await this.deliverInCash(userScope, id, deliverPayload);
      latestDeliveryEvent =
    ...
    finalOrder.paymentStatus = this.resolveNextPaymentStatus(finalOrder);
